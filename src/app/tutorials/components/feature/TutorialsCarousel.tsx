...
import { TbArrowLeft, TbArrowRight, TbVolumeOff, TbVolume } from 'react-icons/tb';

import { useCallback, useEffect, useRef, useState } from 'react';
import { Tutorial } from '@/lib/contentful/types';
...
export default function TutorialsCarousel({ tutorials }: { tutorials: Tutorial[] }) {
    ...
    <section className="embla">
      <div className="embla__viewport" ref={emblaRef}>
        <div className="embla__container">
          {tutorials.map(({ id, name, coach, video }, index) => (
            <div className="embla__slide" key={id}>
              ...
              <div className="coach-info flex flex-col items-center gap-2 lg:gap-3 mt-6 transition-all duration-200">
                <h3 className="tutorial--name leading-snug text-center">{name}</h3>
                <p className="text-grunge50 uppercase polysans-neutral-mono p--sm">
                  {typeof coach !== 'number' ? coach.name : 'N/A'}
                </p>
              </div>
            </div>
          ))}
      ...
      <div className="flex justify-between w-10/12 xl:w-8/12 mx-auto mt-12 lg:mt-0">
        <button
          className="rounded-full border-2 border-demoSmoke p-2 md:p-2.5 hover:bg-demoSmoke hover:text-codGray transition-colors duration-300"
          onClick={toggleMute}
        >
          {isMuted ? (
